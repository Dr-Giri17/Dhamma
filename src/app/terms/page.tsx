    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="prose-dhamma text-ink-soft">
          A short Pāli glossary. Each term carries canonical references so the
          definition is grounded in the texts, not in floating commentary.
        </p>
      </div>
        ...
        {sorted.map((t) => (
          <div key={t.pali} className="card-dhamma">
            <dt className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              ...
              {t.status === "draft" ? (
                <span className="text-xs text-saffron uppercase">draft</span>
              ) : null}
            </dt>
            <dd className="mt-1">
